describe('Angular-Validation Tests:', function () {
  ...
  describe('When clicking on top menu Angular-Validation >> Directive', function () {
    ...
    it('Should navigate to TestingFormDirective page', function () {
      browser.get('http://localhost/Github/angular-validation');

      var anchorLink = $('[name=btn_goto_directive]');
      anchorLink.click();
      ...
      // Assert that the text element has the expected value.
      // Protractor patches 'expect' to understand promises.
      expect(titleElement.getText()).toEqual('Directive');
    });
  ...
  describe('When clicking on top menu Angular-Validation >> Service', function () {
    it('Should navigate to TestingFormService page', function () {
      browser.get('http://localhost/Github/angular-validation');

      var anchorLink = $('[name=btn_goto_service]');
      anchorLink.click();
      browser.waitForAngular();

      // Find the sub-title element
      var titleElement = element(by.css('h3'));
      expect(titleElement.getText()).toEqual('Service');
    });
    ...
    it('Should click on removeValidator2 button blur input2 and make sure there is no error', function () {
      // scroll back to top if we want to be able to click on the English button
      browser.executeScript('window.scrollTo(0,0);').then(function () {
        var elmInput = $('[name=input2]');
        elmInput.click();
        elmInput.sendKeys(protractor.Key.TAB);

        var elmError = $('.validation-input2');
        expect(elmError.getText()).toEqual(errorMessages[0]);

        var elmBtnRemoveValidator2 = $('button[name=btn_RemoveValidator2]');
        elmBtnRemoveValidator2.click();
        browser.waitForAngular();

        elmError = $('.validation-input2');
        expect(elmError.getText()).toEqual('');
      });
    });
